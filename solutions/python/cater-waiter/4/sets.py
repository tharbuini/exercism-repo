...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for diet_name, diet_ingredients in diets.items():
        if dish_ingredients.issubset(diet_ingredients):
...
def tag_special_ingredients(dish):
    ...
    special_ingredient = set(dish[1]) & SPECIAL_INGREDIENTS
...
def compile_ingredients(dishes):
    ...
    for dish_ingredients in dishes:
        master_set.update(dish_ingredients)
...
def separate_appetizers(dishes, appetizers):
    ...
    dishes = set(dishes)
    updated_dishes = list(dishes)
    
    for dish in dishes:
        if dish in appetizers:
            updated_dishes.remove(dish)
